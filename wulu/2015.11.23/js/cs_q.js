...
function setqq(obj,num){
	if (obj.length!=num.length){
		...
		for (var i=0; i<num.length; i++){
			obj[i].innerHTML = "<a target='_blank' href='http://wpa.qq.com/msgrd?v=3&uin="+num[i]+"&site=qq&menu=yes' style='margin-left:6px;color:#474747;font-size:16px;font-weight:bold;'>"+num[i]+"</a>";
		};
	};
};
function settop(id,id2,top){
	...
	obj.style.top = box.style.top = top+150+'px';
};
